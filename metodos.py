...
def ShowGraphics(title):
    colors = ['blue', 'yellow', 'green', 'red']
    plt.title(title)
    plt.xlabel('t')
    plt.ylabel('y')
    
    if title == 'Passo Simples':
        num_of_methods = 4
    else:
        num_of_methods = 3

    i = j = 0
    while i <= (num_of_methods + 2):
        plt.plot(vetores_xy[i], vetores_xy[i+1], 'go', color=colors[j])
        plt.plot(vetores_xy[i], vetores_xy[i+1], 'k:', color=colors[j])
        i += 2
        j += 1
    plt.show()
...
class MetodosPassoSimples:
  def euler(y0, t0, h, qtde, func):
      ...
      if gambiarras[0] != True:
          vetores_xy.append(vetor_x)
          vetores_xy.append(vetor_y)
          gambiarras[0] = True
...
def Adam_Bashforth(ys, t0, h, qtde, func, ordem):
    ...
    vetores_xy.append(t_vetor)
    vetores_xy.append(ys)       
    return ys		
